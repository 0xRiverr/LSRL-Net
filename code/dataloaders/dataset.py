...
class BaseDataSets(Dataset):
    def __init__(
        ...
        # if self.split == "train" or self.split == "mask_levelset":
        if self.split == "train":
        ...
        elif self.split == "mask_levelset":
            with open(self._base_dir + "/train_slices.list", "r") as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
            print("Train total level set {} samples".format(len(self.sample_list)))
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            h5f_levelset = h5py.File(self._base_dir + "/data/png2h5/{}.h5".format(case), "r")
        ...
        sample = {"image": image, "label": label, "mask_levelset": mask_levelset}
...
class WeakStrongAugment(object):
    ...
    def __call__(self, sample):
        image, label, mask_levelset = sample["image"], sample["label"], sample["mask_levelset"]
        ...
        # image_strong = color_jitter(image_strong)
        # # print(image_strong)
        # # label_strong = color_jitter(label).type("torch.FloatTensor")
        # # image_strong = color_jitter(image_strong).type("torch.FloatTensor")
        # image_strong = blur(image_strong.numpy(), p=0.5)
        # # label_strong = blur(label, p=0.5)
        # image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
